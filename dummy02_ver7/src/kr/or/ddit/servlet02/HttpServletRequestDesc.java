...
//		System.out.printf("body length : %d\n", byteLength);
		...
		// parameter
		req.setCharacterEncoding("UTF-8"); // requsetBody에만 적용이 가능하다. - 이말은 post방식만 먹힌다
		// get방식으로는 tomcat의 설정을 건들어야한다. 그 이유는 get방식은 urlEncoding으로 넘겨주기 때문이다.(주소 queryString)
		// queryString은 requsetLine에 해당되기 때문이다. 하지만 useBodyEncodingForURI="True" 설정하면 queryString을 해석하지 않고
		// 뒤로 미룬다. 그래서 이 메서드 하나로 쓸수 있다.
